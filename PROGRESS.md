...
- **Task43 live smoke (2025-12-09):** `npm run build` passed, deployed `snapcase-nb0bhjauq-snapcase.vercel.app` and aliased `dev.snapcase.ai`. Live `/design` loads the Printful embed but CTA stays locked because Printful reports `designValid=false` (“Please add a design!”) even after automated uploads. Latest captures: `Images/diagnostics/task43-design-{desktop,mobile}-2025-12-09T01-41-41-880Z.png`, `Images/diagnostics/task43-edm-live-2025-12-09T01-41-41-880Z.json` (PFUploader processed upload, designStatus errors + 27 non-catalog `selectedVariantIds`); user also provided a real-browser upload screenshot `Images/diagnostics/task43-design-desktop-2025-12-09T20-08-25-user.png` showing the design placed (CTA state not captured). Console logs include Segment CSP warnings (expected). AgentReport updated at `docs/AgentReports/Sprint03-Task43.md`. Next: determine why Printful keeps designValid=false post-upload and force a saved template/valid placement so CTA can unlock.
