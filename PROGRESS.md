...
- **Sprint04-Task10 ESLint config (2025-12-15):** Added the default Next 14 ESLint config (`.eslintrc.cjs`) so `next lint` runs without missing-config errors; no dependency or env changes. Tests: `npm run lint` (react-hooks/exhaustive-deps warning persists at `src/app/design/page.tsx:582`), `npm run build` (same warning).
