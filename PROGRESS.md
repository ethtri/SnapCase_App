...
- **Sprint04-Task14H picker parity simple (2025-12-18):** Simplified the device picker cards (model only, no badges/brand subtitles), hid coming-soon/backorder variants, tuned selection to a thin violet outline + violet-50 tint with compact check chip, cleaned search/tabs spacing with the cloud panel, enlarged the sticky CTA button, and removed redundant banner text. Sorting stays deterministic via brand/displayOrder. Tests: `npm run lint`, `npm run build`. Artifacts: `Images/diagnostics/20251218T093126-picker-parity-simple-desktop.{png,json}`, `Images/diagnostics/20251218T093126-picker-parity-simple-mobile.{png,json}` (prior run: `20251217T214250-*`).
...
### Latest Updates
  ...
  - 2025-12-17: Sprint04-Task14D picker polish landed: removed filter chips, added cloud-tinted spacing, refreshed card selection/disabled styling (violet outline + tint, desaturated coming-soon/backorder), updated helper/CTA copy, and enforced brand/newest ordering via catalog `displayOrder`. Tests: `npm run lint`, `npm run build`. Diagnostics: Images/diagnostics/20251217T232857-picker-polish-{desktop,mobile}.{png,json}. Branch: task/Sprint04-Task14-device-picker-screen. Deployed preview -> https://snapcase-3em4i93vi-snapcase.vercel.app (alias https://dev.snapcase.ai).
  - 2025-12-17: Sprint04-Task14 device picker shipped with brand tabs (Apple/Samsung/Pixel/More), typeahead suggestions, MagSafe/stock/template-fit filters, and a compact catalog error/retry tile; coming-soon devices stay visible but disabled. Catalog gained stock/template metadata plus Pixel/Other entries. Docs/AgentReport refreshed. Diagnostics: Images/diagnostics/2025-12-17T02-54-41-749Z-picker-{desktop,mobile}.{png,json}. Tests: `npm run lint`, `npm run build`. Manual smoke: Samsung selection unlocked CTA; Pixel cards remain disabled (coming soon). Compare: https://github.com/ethtri/SnapCase_App/compare/main...task/Sprint04-Task14-device-picker-screen.
...
- 2025-12-17 - Deployed Sprint04-Task14C picker polish to preview `https://snapcase-nb3vubwbf-snapcase.vercel.app` and aliased `https://dev.snapcase.ai` for sponsor testing. Verification: curl design 200, lint/build prior to deploy. Diagnostics: Images/diagnostics/20251217T212200-picker-clean-{desktop,mobile}.{png,json}. Compare: https://github.com/ethtri/SnapCase_App/compare/main...task/Sprint04-Task14-device-picker-screen.
