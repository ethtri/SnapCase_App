...
### Latest Updates
  ...
  - 2025-12-18: Sprint04-Task17 design shell cleanup (branch `task/Sprint04-Task17-design-shell-cleanup`). Removed the visible blocker badge while keeping the picker guard overlay (now transparent with an sr-only label), added header + chip Change device controls, merged status into the design summary card with tokenized chip/metadata and a shell skeleton, and tweaked iframe height/padding so the Printful toolbar is fully visible. Diagnostics: Images/diagnostics/20251218T113816-design-shell-{desktop,mobile}.png. Tests: `npm run lint`, `npm run build`. AgentReport: docs/AgentReports/Sprint04-Task17.md. Compare: https://github.com/ethtri/SnapCase_App/compare/main...task/Sprint04-Task17-design-shell-cleanup.
