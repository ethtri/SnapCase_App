...
- **Sponsor UX feedback (2025-12-15):** Sponsor flagged a visible blocker overlay in the Printful iframe, an unnecessary Flow bar, desktop-only “Detect my device,” and an oversized proof rail. Mitigation shipped in Task63: product-tab guard made transparent (still intercepts clicks to protect variant lock), Flow header condensed, detect button `md:hidden`, proof rail collapsed to a status card. Next: confirm acceptance; replace guard with a first-party disable flag if Printful exposes it, and consider an optional collapse for the rail.
