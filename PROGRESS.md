...
- **Sprint04-Task12 issue triage (2025-12-16):** Restored `.eslintrc.cjs` (extends `next`, `next/core-web-vitals`) so lint can run on this branch. Updated the CTA `useMemo` deps to include `guardrailSummary.message` (`[edmSnapshot, guardrailSummary.message, ownershipHelper]`). `npm run lint` and `npm run build` now complete; both surface the known `react-hooks/exhaustive-deps` warning on the CTA/checkout `useCallback` dependency list (left as-known).
