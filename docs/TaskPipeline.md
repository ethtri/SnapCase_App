...
| Sprint04-Task20 | DONE | Simplified the `/design` summary card into a single status chip + helper showing only device/variant, finish, and price; removed last-saved/attempt fields while keeping CTA gating intact. Diagnostics: `Images/diagnostics/20251218T153607-design-summary-{desktop,mobile}.png`. Tests: `npm run lint`; `npm run build`. See `docs/AgentReports/Sprint04-Task20.md`. |
