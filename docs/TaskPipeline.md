...
| Sprint04-Task20 | DONE | Rebased on Task18 picker/shell and reapplied the simplified summary card (single status chip/helper showing only device/variant, finish, price). Diagnostics: `Images/diagnostics/20251218T155914-design-{picker,shell}-{desktop,mobile}.png`. Tests: `npm run lint`; `npm run build`. Preview: `https://snapcase-pxrwcquta-snapcase.vercel.app`. See `docs/AgentReports/Sprint04-Task20.md`. |
